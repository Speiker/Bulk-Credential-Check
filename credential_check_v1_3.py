...
user_message = Fore.YELLOW + "Credential Check - v1.3\n\n" + Fore.WHITE
print(user_message)


# Limits the number of simultaneous threads
maxthreads = 50
...
def connection_test():
    ...
    # close log after writing header; additional logs will be append
    file.close()

    # This loop will test SSH then Telnet connections to every device in the list
    threads = []
    for device in device_list:
        my_thread = threading.Thread(target=test, args=(device,))
        my_thread.start()
